...
** Last update Sun Jul 15 18:37:32 2012 Thomas Duplomb
*/
      ...
      if ((plyr_lnk = get_link(g_info.users, i)))
	{
	  ...
	  if (plyr->x == x && plyr->y == y && plyr->lvl == lvl &&
	      (plyr->type == TPLAYER || plyr->type == TFORMER_GHOST))
	    {
	      ++plyr->lvl;
	      check_end_game(plyr);
	      snprintf(msg, sizeof(msg), "Level up! Player #%d (%d-%d) nb: %d"
		       " is level %d!\n",
		       plyr->id, plyr->x, plyr->y, g_info.map->cases[y][x].elements[PLAYER], plyr->lvl);
	      log_msg(stdout, msg);
	    }
	}
      ++i;
